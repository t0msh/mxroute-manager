...
2. Install dev deps and run the test suite:
   ...
   That runs Python tests and `static/js/*.test.js` via Node. You need Node 18+ locally; CI uses Node 22. Details: [docs/testing.md](docs/testing.md).
...
| Tests | `tests/`, `static/js/*.test.js` | See [docs/testing.md](docs/testing.md) |
...
Split files when they grow (the frontend already follows this; see [docs/frontend-app-scripts.md](docs/frontend-app-scripts.md)).

## Python

- Match surrounding style: plain functions, explicit imports, no framework magic beyond Flask.
- Patch mocks at the import site the route uses (documented in [docs/testing.md](docs/testing.md)).
...
If you add or reorder app scripts, update **both** `templates/index.html` and [docs/frontend-app-scripts.md](docs/frontend-app-scripts.md). `init.js` loads late on purpose.
...
User guides live in `docs/`. The site is built with MkDocs Material (`mkdocs serve` for local preview). If you add a feature, add or extend the guide that a user would actually open. Keep [docs/README.md](docs/README.md) honest if you add a new top-level doc.
...
| Guide | Topic |
| --- | --- |
| [docs/testing.md](docs/testing.md) | Fixtures, layers, mocking |
| [docs/frontend-app-scripts.md](docs/frontend-app-scripts.md) | Script load order |
| [docs/api.md](docs/api.md) | HTTP API for automation |
