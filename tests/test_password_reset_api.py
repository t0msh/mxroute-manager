...
def test_reset_request_sends_email_when_recovery_exists(
...
def test_reset_request_uses_branded_from_when_portal_enabled(
    fresh_db, client, db_connection
):
    ...
    mock_send.assert_called_once()
    assert (
        mock_send.call_args.kwargs["from_address"] == "Example Org <reset@example.com>"
    )
...
def test_reset_confirm_updates_mailbox_password(fresh_db, client):
    ...
    with (
        patch("routes.password_reset.is_password_reset_available", return_value=True),
        patch(
            "routes.password_reset.mx_request_raw",
