...
def test_deploy_missing_dns_skips_records_already_passing():
    ...
    with (
        ...
        result = deploy_missing_dns_to_cf(
            DOMAIN, record_types=["verification", "mx", "spf", "dkim"]
        )
...
def test_deploy_missing_dns_audits_when_records_fixed():
    health = _health(
        checks={"verification": {"status": "fail"}, "mx": {"status": "pass"}}
    )
    patches = _fix_patches(health)
    with (
        patches[0],
        ...
        patches[5],
        patches[6],
        patches[7] as mock_audit,
    ):
        deploy_missing_dns_to_cf(DOMAIN, record_types=["verification"])

    mock_audit.assert_called_once_with(
        "dns.fix", target=DOMAIN, records=["verification"], outcome="updated"
    )
...
def test_deploy_missing_dns_audits_no_changes_when_deploy_skips():
    ...
    mock_audit.assert_called_once_with(
        "dns.fix", target=DOMAIN, records=[], outcome="no_changes"
    )
...
def test_deploy_missing_dns_treats_updated_as_fixed():
    ...
    mock_audit.assert_called_once_with(
        "dns.fix", target=DOMAIN, records=["spf"], outcome="updated"
    )
