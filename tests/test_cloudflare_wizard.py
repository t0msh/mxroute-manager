...
def _wizard_patches(*, register_result="added", deploy_result="added"):
    return (
        ...
        patch(
            "routes.cloudflare.get_mxroute_verification_record",
            return_value=VERIFY_RECORD,
        ),
        ...
        patch(
            "routes.cloudflare.register_domain_on_mxroute", return_value=register_result
        ),
...
def test_cf_setup_requires_cloudflare_credentials(fresh_db, client, admin_token):
    with (
        patch("routes.cloudflare.cf_is_configured", return_value=False),
        patch("routes.cloudflare.ensure_cf_zone") as mock_zone,
    ):
        response = client.post(
            "/api/cloudflare/setup",
...
def test_cf_setup_happy_path(fresh_db, client, admin_token):
    ...
    with (
        ...
        patches[1],
        patches[2],
        patches[3] as mock_deploy,
        patches[4] as mock_register,
        patches[5],
        patches[6] as mock_audit,
    ):
        response = client.post(
            "/api/cloudflare/setup",
            headers=auth_post_headers(admin_token),
...
def test_cf_setup_succeeds_when_mxroute_already_registered(
    fresh_db, client, admin_token
):
...
def test_setup_health_requires_dns_permission(
    fresh_db, client, dns_delegate_token, db_connection
):
    health = {
        "domain": DOMAIN,
        "overall": "degraded",
...
def test_dns_delegate_can_read_health(fresh_db, client, dns_delegate_token):
    ...
    with (
        patch("routes.cloudflare.mx_request_raw", side_effect=mx_raw_side_effect),
